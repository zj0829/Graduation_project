...
class LLMInterface:
    ...
    def generate_report(self, test_plan, results, analysis):
        ...
        prompt = (
            ...
            "2. 风险评分：在执行摘要后给出整体风险评分（0-100分，100为最危险），用表格展示各维度的评分：\n"
            "   | 评估维度 | 评分(0-100) | 说明 |\n"
            "   |---------|------------|------|\n"
            "   | 漏洞严重性 | xx | ... |\n"
            "   | 攻击面暴露 | xx | ... |\n"
            ...
            "   | 配置安全性 | xx | ... |\n"
            "   | **综合评分** | **xx** | ... |\n"
            "3. 测试过程（300-400字）：详细描述测试步骤、使用的工具、执行方法和过程。\n"
            "4. 发现的漏洞（每个漏洞200-300字）：针对每个漏洞，详细描述：\n"
            "   - 漏洞名称、类型、严重程度（Critical/High/Medium/Low/Info）\n"
            "   - CVE编号（如能确定，格式：CVE-YYYY-NNNNN）\n"
            "   - CVSS评分（如能确定）\n"
            "   - 影响范围、技术细节和证据\n"
            "   - OWASP Top 10分类（如A01:2021-Broken Access Control）\n"
            "5. 风险评估（200-250字）：基于漏洞的严重程度和影响范围，评估总体风险等级。\n"
            "6. 修复建议（每个建议100-150字）：针对每个漏洞提供具体、可操作的修复建议。\n"
            "7. 附录：列出使用的测试工具及版本，测试环境信息。\n\n"
            "【语言格式要求】\n"
